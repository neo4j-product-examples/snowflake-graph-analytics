...
SELECT neo4j_graph_analytics.gds.betweenness_centrality('path_graph', {
    'mutateProperty': 'flow_control'
           });

SELECT neo4j_graph_analytics.gds.eigenvector_centrality('path_graph', {
    'mutateProperty': 'transitive_influence',
                                                        'relationshipWeightProperty': 'FLIGHT_COUNT'
           });

-- run louvain community detection
SELECT neo4j_graph_analytics.gds.louvain('path_graph', {
    'relationshipWeightProperty': 'FLIGHT_COUNT',
                                         'mutateProperty': 'inferred_region'
           });

-- write relationships back ot table
-- write centrality measrues
SELECT neo4j_graph_analytics.gds.write_nodeproperties('path_graph',
           {'nodeProperties': ['flow_control', 'transitive_influence', 'inferred_region'], 'table': 'freight_network.public.airport_metrics'});
SELECT * FROM AIRPORT_METRICS;
-- ///////////////////////////////////////////////////////////
-- 4) CLEANUP NEO4J
-- ///////////////////////////////////////////////////////////
